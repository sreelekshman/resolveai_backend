...
class CloseComplaintTool {
    name = "close_complaint"
    description = "Close a complaint by its ID";
    schema = {
        type: "object",
        properties: {
    ...
    async call({ complaintId }) {
        try {
            if (!JWTtoken) {
                return "❌ Please authenticate first using your college credentials before closing a complaint.";
            }
            
            const response = await axios.patch(`http://localhost:5000/api/complaints/close/${complaintId}`, {}, {
                headers: { 'Authorization': `Bearer ${JWTtoken}` }
            });
            return `Complaint with ID ${complaintId} closed successfully.`;
        } catch (error) {
            console.error('Close complaint error:', error.response?.data || error.message);
            return `❌ Error closing complaint: ${error.response?.data?.error || error.message}`;
        }
    }
}

const tools = [new SubmitComplaintTool(), new UserAuthTool(), new FetchComplaintsTool(), new CloseComplaintTool()];
...
class langgraphService {
    static async LLMResponse(messageText, threadId="default_thread") {
        // Check if messageText is provided and is a string
        ...
        try {
            ...
            // Return the content, with fallback for empty responses
            return lastMessage.content || "I'm sorry, I couldn't generate a response.";
        } catch (error) {
